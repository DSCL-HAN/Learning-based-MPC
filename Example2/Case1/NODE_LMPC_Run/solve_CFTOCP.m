...
for i=1:tn_x
    ...
    NODE_qr(:,i+1) = rk5(NODE_qr(:,i),ti*i,ti);
    opti.subject_to(x(:,i+1)==F(x(:,i),[sam_u;NODE_qr(1,i);NODE_qr(2,i)]));
%     sam_u
%     opti.subject_to(x(:,i+1)==F(x(:,i),sam_u));
    t = t + 1;
end
...
%     opti.subject_to(-max_u <= u(1,i) <= max_u);
%     opti.subject_to(-max_u <= u(2,i) <= max_u);
% end
P = eye(4);
r = 0.0001;
opti.subject_to(x(:,1)==p);
% opti.subject_to(x(:,tn_x+1)==[0;0;0;0]);
