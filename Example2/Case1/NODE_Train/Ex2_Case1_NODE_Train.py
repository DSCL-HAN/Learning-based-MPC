...
def EX2_C1_Trajectory(z, t):
    ...
    dz[:,0] = W_1*z[:,1]
    dz[:,1] = W_2*z[:,0] + W_3*z[:,1] + W_4*torch.sin(t*0.5)
    
    return dz

def rk4_step(z, t, h):
    k1 = EX2_C1_Trajectory(z, t)*h
    k2 = EX2_C1_Trajectory(z + 0.5*k1, t + 0.5*h)*h
    k3 = EX2_C1_Trajectory(z + 0.5*k2, t + 0.5*h)*h
    k4 = EX2_C1_Trajectory(z + k3,     t + h)*h
...
class LinearODEF_true(ODEF):
    ...
    def forward(self, x, t):
        ...
        z_out[:,0] = W_1*x[:,1]
        z_out[:,1] = W_2*x[:,0] + W_3*x[:,1] + W_4*torch.sin(t*0.5)
...
class LinearODEF_train(ODEF):
    ...
    def forward(self, x, t):
        ...
        z_out[:,0] = f_w[0,0]*x[:,1]
        z_out[:,1] = f_w[0,1]*x[:,0] + f_w[1,0]*x[:,1] + f_w[1,1]*torch.sin(t*0.5)
...
PATH = "./Ex2_Case1_NODE_model.pt"
torch.save(ode_trained.state_dict(), PATH)

now = datetime.datetime.now()
date_str = now.strftime("%y%m%d")
filename1 = f"Ex2_Case1_W1_{date_str}.mat"
filename2 = f"Ex2_Case1_W2_{date_str}.mat"
filename3 = f"Ex2_Case1_W3_{date_str}.mat"
filename4 = f"Ex2_Case1_W4_{date_str}.mat"
...
sio.savemat('Ex2_Case1_NODE_trajectory.mat', {"data": z_node_array })
sio.savemat('Ex2_Case1_True_trajectory.mat', {"data": z_true_array })
