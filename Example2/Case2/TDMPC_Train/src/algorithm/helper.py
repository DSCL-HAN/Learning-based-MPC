...
def enc(cfg):
	...
	if cfg.modality == 'pixels':
		...
		layers = [NormalizeImg(),
				  ...
				  nn.Conv2d(cfg.num_channels, cfg.num_channels, 3, stride=2), nn.ReLU()]   # 합성곱 layer
	...
	return nn.Sequential(*layers) # layer 1, layer 2...등 여러 객체를 여러 매개변수로 넘겨줄 때 *laeyrs 사용함
...
def mlp(in_dim, mlp_dim, out_dim, act_fn=nn.ELU()):
	...
	return nn.Sequential(
		...
		nn.Linear(mlp_dim[1], out_dim)) # mlp 생성
...
class ReplayBuffer():
	...
	def __init__(self, cfg):
		self.cfg = cfg
		self.device = torch.device(cfg.device)
		self.capacity = min(cfg.train_steps, cfg.max_buffer_size) # capacity는 학습시 중요함!
		dtype = torch.float32 if cfg.modality == 'state' else torch.uint8
		obs_shape = cfg.obs_shape if cfg.modality == 'state' else (3, *cfg.obs_shape[-2:])
		self._obs = torch.empty((self.capacity+1, *obs_shape), dtype=dtype, device=self.device) # memory만 할당함 
		# obs_shape가 (3, 64, 64)라는 튜플이라면, (self.capacity + 1, *obs_shape)는 (self.capacity + 1, 3, 64, 64)로 언패킹(확장)됩
		# 첫 번째 차원(예: self.capacity + 1)은 버퍼에 저장될 데이터의 최대 개수, 그 뒤 따라오는 차원들(obs_shape)은 실제 관측(observation)의 차원 정보를 나타냅니다.
