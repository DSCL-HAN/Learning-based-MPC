...
class TDMPC():
	...
	def plan(self, obs, eval_mode=False, step=None, t0=True):
		...
		if num_pi_trajs > 0:
			...
			z = self.model.h(obs).repeat(num_pi_trajs, 1) # learned model
			for t in range(horizon):
				pi_actions[t] = self.model.pi(z, self.cfg.min_std) # t에 따라서 pi_action 값을 결정함
				z, _ = self.model.next(z, pi_actions[t])

		# Initialize state and parameters
		z = self.model.h(obs).repeat(self.cfg.num_samples+num_pi_trajs, 1)
		mean = torch.zeros(horizon, self.cfg.action_dim, device=self.device) # a를 학습하기 위하여 mean, std 학습
		...
		for i in range(self.cfg.iterations):
			actions = torch.clamp(mean.unsqueeze(1) + std.unsqueeze(1) * \
				torch.randn(horizon, self.cfg.num_samples, self.cfg.action_dim, device=std.device), -1, 1) # action nomralize
			if num_pi_trajs > 0:
	...
	def update(self, replay_buffer, step):
		...
		consistency_loss, reward_loss, value_loss, priority_loss = 0, 0, 0, 0 # priority loss 의미 모름
		for t in range(self.cfg.horizon):
			...
			# Losses
			rho = (self.cfg.rho ** t) # rho = 0.5; it is fixed.
