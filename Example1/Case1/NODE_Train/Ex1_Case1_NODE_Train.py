...
def EX1_C1_Trajectory(z, t):
    ...
    dz[:, 0] = W_1*z[:,0] + W_2*z[:,1]
    dz[:, 1] = W_3*z[:,0] + W_4*torch.cos(t*0.5)

    return dz

def rk4_step(z, t, h):
    k1 = EX1_C1_Trajectory(z, t)*h
    k2 = EX1_C1_Trajectory(z + 0.5*k1, t + 0.5*h)*h
    k3 = EX1_C1_Trajectory(z + 0.5*k2, t + 0.5*h)*h
    k4 = EX1_C1_Trajectory(z + k3,     t + h)*h
...
class LinearODEF_true(ODEF):
    ...
    def forward(self, z, t):
        ...
        z_out[:, 0] = W_1*z[:,0] + W_2*z[:,1]
        z_out[:, 1] = W_3*z[:,0] + W_4*torch.cos(t*0.5)
...
class LinearODEF_train(ODEF):
    def __init__(self, W):
        ...
        self.lin1 = nn.Linear(2, 2, bias=False).to(device=device, dtype=data_type)
        self.lin1.weight = nn.Parameter(W)

    def forward(self, z, t):
        z_out = torch.zeros(1,2).to(device=device, dtype=data_type)
        f_w = self.lin1.weight.to(device=device, dtype=data_type)
        
        z_out[:,0] = f_w[0,0]*z[:,0] + f_w[0,1]*z[:,1]
        z_out[:,1] = f_w[1,0]*z[:,0] + f_w[1,1]*torch.cos(t*0.5)
...
class RandomLinearODEF(LinearODEF_train):
    def __init__(self):
        super(RandomLinearODEF, self).__init__(torch.randn(2, 2).to(device=device, dtype=data_type)/2.) 
...
def conduct_experiment(ode_true, ode_trained, n_steps, name, plot_freq, Tf, n_points, z_initial, min_delta_time, max_delta_time, Nd):
    ...
    # obs = obs + torch.randn_like(obs) * 0.00175
    obs_temp = obs
    optimizer = torch.optim.Adam(ode_trained.parameters(), lr=1e-2)
    ...
    for i in range(n_steps):
        ...
        for p in ode_trained.parameters():
                    weight_nn1 = p.detach().cpu().numpy()[0,0]
                    weight_nn2 = p.detach().cpu().numpy()[0,1]
                    weight_nn3 = p.detach().cpu().numpy()[1,0]
                    weight_nn4 = p.detach().cpu().numpy()[1,1]

        if i==0: 
            weight_temp1 = [weight_nn1]
            weight_temp2 = [weight_nn2]
            weight_temp3 = [weight_nn3]
            weight_temp4 = [weight_nn4]
        
        weight_temp1.append(weight_nn1) 
        weight_temp2.append(weight_nn2) 
        weight_temp3.append(weight_nn3) 
        weight_temp4.append(weight_nn4) 
    ...
    return weight_temp1, weight_temp2, weight_temp3, weight_temp4,obs_temp, ts_temp, Loss_NODE_out
...
weight_temp1, weight_temp2, weight_temp3, weight_temp4, obs_temp_out, ts_temp_out, Loss_NODE_out = conduct_experiment(
...
PATH = "./Ex1_Case1_NODE_model.pt"
torch.save(ode_trained.state_dict(), PATH)

now = datetime.datetime.now()
date_str = now.strftime("%y%m%d")
filename1 = f"Ex1_Case1_W1_{date_str}.mat"
filename2 = f"Ex1_Case1_W2_{date_str}.mat"
filename3 = f"Ex1_Case1_W3_{date_str}.mat"
filename4 = f"Ex1_Case1_W4_{date_str}.mat"

sio.savemat(filename1, {"data": weight_temp1 })
sio.savemat(filename2, {"data": weight_temp2 })
sio.savemat(filename3, {"data": weight_temp3 })
sio.savemat(filename4, {"data": weight_temp4 })
...
sio.savemat('Ex1_Case1_NODE_trajectory.mat', {"data": z_node_array })
sio.savemat('Ex1_Case1_True_trajectory.mat', {"data": z_true_array })
