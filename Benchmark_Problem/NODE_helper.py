...
class LinearODEF_true(ODEF):
    ...
    def forward(self, x, t):
        ...
        z_out[:, 0] = -0.1*x[:,0] - 1.0*x[:,1]
        z_out[:, 1] = 1.0*x[:,0] - 0.1*x[:,1]
...
#     obs = obs + torch.randn_like(obs) * 0.005
    ...
    (line_z1_true,) = ax_traj.plot(t_, obs_np[:,0], 'r-', label='True $z_{1}$')
    (line_z2_true,) = ax_traj.plot(t_, obs_np[:,1], 'b--', label='True $z_{2}$')

    (line_z1_pred,) = ax_traj.plot(t_, obs_np[:,0]*0.0, 'm-', label='NODE $z_{1}$')
    (line_z2_pred,) = ax_traj.plot(t_, obs_np[:,1]*0.0, 'c--', label='NODE $z_{2}$')
    ...
    for i in range(n_steps):
        ...
        for j in range(len(obs_)-1):
            z0_j = obs_[j]        # shape=(1,2)
            t0_j = ts_[j]         # shape=(1,1,1)
            t1_j = ts_[j+1]       # shape=(1,1,1)
            # 한 스텝 적분
            z1_pred = ode_solve(z0_j, t0_j, t1_j, f = ode_trained.func) 
            z1_true = obs_[j+1]   # shape=(1,2)
        ...
        if i % plot_freq == 0:
            ...
            pred_np = pred_full[:,0,:].cpu().numpy()  # shape=(n_points,2)
            
            line_z1_pred.set_ydata(pred_np[:,0])
            line_z2_pred.set_ydata(pred_np[:,1])
            ax_traj.relim()
